...
export const MainSidebar = ({ collapseSidebar = true }) => {
      ...
      menuData.map((v) => {
         if (v?.children?.length > 0) {
            ...
            v.children.map((i) => {
               if (i?.children?.length > 0) {
                  ...
                  submenu.push(
                     getItem(
                        i.url ? (
                           <Link href={i.url} style={{ color: 'unset' }}>
                              {i.title}
                           </Link>
                        ) : (
            ...
            listMenu.push(
               getItem(
                  v.url ? (
                     <Link href={v.url} style={{ color: 'unset' }}>
                        {v.title}
                     </Link>
                  ) : (
                     <Link href="#" style={{ color: 'unset' }}>
   ...
   return (
      <Sider width={300} collapsible={true} collapsed={collapsed} theme="light" onCollapse={(value) => setCollapsed(value)}>
         <RouteLink to={'/'}>
