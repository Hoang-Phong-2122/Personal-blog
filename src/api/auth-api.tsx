...
class AuthApi {
    ...
    async login({username, password}): Promise<User> {
        ...
        await axios(requestOptions).then((rp) => {
            console.log('rp', rp);
            if (rp.status == 200) {
                if (rp.data.status == 1){
                ...
                responseData = {status: 0, message: 'Tài khoản hoặc mật khẩu không hợp lệ'};
            } else {
            }
            ...
            console.log('login error: ', error);
                if (error.response) {
                    ...
                    if (error.request.timeout) {
                        responseData = {status: 0, message: 'Kết nối tới hệ thống thất bại. Vui lòng thử lại'};
                    } else {
                        responseData = {status: 0, message: 'Kết nối wifi/4G/3G/GPRS bị gián đoạn. Vui lòng kiểm tra lại'};
                    }
